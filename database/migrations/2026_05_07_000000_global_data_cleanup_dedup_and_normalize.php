    ...
    public function up(): void
    {
        ...
        try {
                    ...
                    $deleted = DB::delete(
                        "DELETE FROM plannings
                         WHERE id IN (
                            ...
                            AND t1.date = t2.date
                            AND (t1.tenant_id = t2.tenant_id OR (t1.tenant_id IS NULL AND t2.tenant_id IS NULL))
                         )"
                    );
                } else {
                    $deleted = DB::delete(
                ...
                if ($driver === 'sqlite') {
                    $deleted = DB::delete(
                        "DELETE FROM pointages
                         WHERE id IN (
                            SELECT t1.id FROM pointages t1
            ...
            if (Schema::hasTable('compteurs_temps') && Schema::hasColumn('compteurs_temps', 'tenant_id')) {
                if ($driver === 'sqlite') {
                    $deleted = DB::delete(
                        "DELETE FROM compteurs_temps
                         WHERE id IN (
                            SELECT t1.id FROM compteurs_temps t1
            ...
            if (Schema::hasTable('droits_absences') && Schema::hasColumn('droits_absences', 'tenant_id')) {
                if ($driver === 'sqlite') {
                    $deleted = DB::delete(
                        "DELETE FROM droits_absences
                         WHERE id IN (
                            SELECT t1.id FROM droits_absences t1
