...
<div class="coverage-bar">
    <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;">
        ...
        <span style="font-size:14px;font-weight:800;color:{{ $stats['coverage_pct'] >= 100 ? 'var(--green)' : ($stats['coverage_pct'] >= 50 ? 'var(--warning)' : 'var(--danger)') }};"
              id="statCoverage">{{ $stats['coverage_pct'] }}%</span>
        <span style="font-size:11px;color:var(--text-light);">({{ $stats['active_employees'] }} employé(s))</span>
    </div>
</div>
    ...
    <table class="codes-table" id="codesTable">
        ...
        <tbody>
            ...
            <tr class="code-row"
                ...
                <td style="font-size:10px;color:var(--text-light);">{{ str_pad($i + 1, 2, '0', STR_PAD_LEFT) }}</td>
                <td>
                    @if($code->user)
                    <div class="user-row">
                ...
                <td style="font-size:11px;color:var(--text-muted);">{{ $code->assigned_at?->format('d/m/Y') ?? '—' }}</td>
                <td style="font-size:11px;color:{{ $isUsedOnce ? 'var(--green)' : 'var(--text-muted)' }};font-weight:{{ $isUsedOnce ? '600' : '400' }};"
