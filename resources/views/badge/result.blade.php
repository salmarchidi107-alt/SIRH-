{{-- resources/views/badge/result.blade.php --}}
...
<div class="result-wrap">
  ...
  <div class="result-icon-wrap">
    <div class="result-ripple {{ $typeClass }}"></div>
    <div class="result-ripple {{ $typeClass }}" style="animation-delay:.5s;"></div>
    <div class="result-icon {{ $typeClass }}">{{ $typeIcon }}</div>
  </div>
  ...
  <div class="result-card">
    ...
    @if($employee)
      <div class="result-name">{{ $employee->first_name }} {{ $employee->last_name }}</div>
      <div class="result-dept">{{ $employee->department ?? '—' }} · {{ $employee->matricule }}</div>
    @else
      <div class="result-name">{{ auth()->guard('badge')->user()->name ?? '—' }}</div>
      <div class="result-dept">—</div>
    @endif

    {{-- Heure principale --}}
    <div class="result-time-label">{{ $timeLabel }}</div>
    <div class="result-time-big {{ $typeClass }}">{{ $displayTime }}</div>
    <div class="result-time-label" style="margin-top:4px;">
      {{ $nowCasa->isoFormat('dddd D MMMM YYYY') }}
    </div>

    {{-- Grid shift : Entrée / Sortie / Total --}}
    <div class="divider"></div>
    <div class="shift-grid">
      <div class="shift-cell">
        <div class="shift-cell-label">📥 Entrée</div>
        <div class="shift-cell-value">{{ $todayShift['first_entree'] ?? '—' }}</div>
      </div>
      <div class="shift-cell">
        <div class="shift-cell-label">📤 Sortie</div>
        <div class="shift-cell-value heure-sortie">{{ $todayShift['last_sortie'] ?? '—' }}</div>
      </div>
      <div class="shift-cell">
        <div class="shift-cell-label">⏱ Total</div>
        <div class="shift-cell-value total-heures">{{ $todayShift['total_human'] ?? '0h 0m' }}</div>
      </div>
    </div>

    {{-- ✅ CORRIGÉ : section pause visible dès que pause_start OU pause_end est rempli --}}
    @if($showPauseSection)
    <div class="divider"></div>
    <div class="shift-grid">
      <div class="shift-cell">
...
const timer = setInterval(() => {
  ...
  if (seconds <= 0) {
    clearInterval(timer);
    window.location.href = '{{ route('badge.pointage') }}';
  }
}, 1000);
