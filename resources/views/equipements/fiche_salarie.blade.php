...
@section('title', 'Fiche équipements — ' . $employee->first_name . ' ' . $employee->last_name)
@section('page-title', 'Fiche patrimoine salarié')
...
<div style="display:flex;align-items:center;gap:8px;margin-bottom:16px;font-size:13px">
 ...
 <span style="color:#0f172a;font-weight:500">{{ $employee->first_name }} {{ $employee->last_name }}</span>
</div>
...
<div class="eq-card" style="margin-bottom:20px">
 ...
 @endphp
 <div class="eq-avatar" style="width:54px;height:54px;font-size:18px">{{ $initiales }}</div>
 <div style="flex:1">
 ...
 <span>{{ $employee->position ?? $employee->poste ?? '—' }}</span>
 @if($employee->department ?? $employee->departement ?? '')
 <span style="color:#cbd5e1">·</span>
 <span>{{ $employee->department ?? $employee->departement }}</span>
 @endif
 @if($employee->employee_number ?? $employee->matricule ?? '')
 <span style="color:#cbd5e1">·</span>
 <span class="mono" style="font-size:12px">{{ $employee->employee_number ?? $employee->matricule }}</span>
 @endif
 </div>
 </div>
 ...
 @endif
 <a href="{{ route('equipements.index', ['tab' => 'affecter', 'employee_id' => $employee->id]) }}"
 ...
 <div class="stat-val" style="font-size:18px">{{ number_format($metrics_salarie['valeur_confiee'], 0, ',', ' ') }} <span style="font-size:12px;font-weight:500;color:#64748b">MAD</span></div>
 </div>
 <div class="eq-stat-card amber">
 <div class="stat-label">Dernière affectation</div>
 <div class="stat-val" style="font-size:16px">{{ optional($metrics_salarie['derniere_affectation'])->format('d/m/Y') ?? '—' }}</div>
 </div>
 <div class="eq-stat-card green">
 <div class="stat-label">Décharges signées</div>
 <div class="stat-val">{{ $metrics_salarie['decharges_signees'] }}</div>
 </div>
 </div>
</div>
...
<div class="eq-card">
 ...
 <tr>
 <td class="mono" style="font-weight:600;color:#0f172a">{{ $aff->equipement->reference ?? '—' }}</td>
 <td style="font-weight:500">{{ $aff->equipement->designation ?? '—' }}</td>
 <td>
 @if($aff->equipement)
 <span class="{{ $aff->equipement->categorie_color ?? 't-gray' }}">{{ $aff->equipement->categorie }}</span>
 @else —
 @endif
 </td>
 ...
 <td>
 @if($aff->equipement)
 <span class="{{ $aff->equipement->etat_color ?? 't-gray' }}">{{ $aff->etat_remise }}</span>
 @else —
 @endif
 </td>
 <td style="text-align:right;font-weight:600">{{ number_format($aff->equipement->valeur_acquisition ?? 0, 0, ',', ' ') }}</td>
 <td>
...
<div class="eq-card">
 ...
 <td>
 @if($h->statut === 'Restitué')
 <span class="t-amber">Restitution</span>
 @elseif($h->statut === 'Perdu')
 ...
 <td>
 <span style="font-weight:500">{{ $h->equipement->designation ?? '—' }}</span>
 <span class="mono" style="color:#94a3b8;margin-left:4px">{{ $h->equipement->reference ?? '' }}</span>
 </td>
 <td>
 @if($h->equipement)
 <span class="{{ $h->equipement->etat_color ?? 't-gray' }}">{{ $h->etat_remise }}</span>
 @else —
 @endif
 </td>
 <td style="color:#64748b;font-size:12px">{{ $h->observations ?? '—' }}</td>
 </tr>
 @empty
 <tr>
