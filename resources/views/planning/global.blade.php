...
<div class="filters-bar" style="margin-bottom: 20px;">
    <form method="GET" action="{{ route('planning.global') }}" style="display:flex;gap:12px;flex-wrap:wrap;align-items:center">
        <div style="display:flex;align-items:center;gap:8px">
            <a href="{{ route('planning.global', ['month' => $month - 1, 'year' => $month == 1 ? $year - 1 : $year]) }}" class="btn btn-sm btn-outline">< Mois precedent</a>
        ...
        <div style="display:flex;gap:8px;margin-left:auto">
            ...
            @if(isset($search) && $search || isset($department) && $department)
                <a href="{{ route('planning.global', ['month' => $month, 'year' => $year]) }}" class="btn btn-outline btn-sm">Reinit.</a>
            @endif
        </div>
    </form>
</div>
