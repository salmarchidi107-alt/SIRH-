{{-- resources/views/pointage/badges-pin.blade.php --}}
@extends('layouts.app')
...
<div class="pin-wrap">
    ...
    <div class="pin-topbar">
        ...
        <div style="display:flex;align-items:center;gap:.75rem;">
            <a href="{{ route('pointage.index') }}" class="pin-back">
                ← Retour Pointage
            </a>
        </div>
    </div>
    ...
    <div class="pin-content screen-only" id="pinContent">
        ...
        <div class="dept-card" data-dept="{{ $dept }}" id="dept-{{ Str::slug($dept) }}">
                ...
                <table class="pin-table">
                    ...
                    <tbody>
                        ...
                        <tr id="emp-row-{{ $emp->id }}" class="emp-row"
                            ...
                            <td>
                                <span class="emp-matricule">{{ $emp->matricule ?? '—' }}</span>
                            </td>
                            <td>
                                <div class="pin-display">
                                    <span class="pin-code" id="pin-{{ $emp->id }}" data-emp-id="{{ $emp->id }}">
                                        {{ $emp->plain_pin ?? '——' }}
                                    </span>
                                </div>
                            </td>
    ...
    {{-- Section d'impression (TOUS les départements) --}}
    <div class="print-all-content print-only">
        @forelse($allByDept as $dept => $employees)
            ...
            <div class="dept-header" style="display:flex;align-items:center;justify-content:space-between;padding:.85rem 1.25rem;background:linear-gradient(135deg, var(--pin-purple-bg) 0%, var(--pin-teal-bg) 100%);border-bottom:1px solid var(--pin-border);">
                <div style="display:flex;align-items:center;gap:10px;">
                    <div style="width:34px;height:34px;border-radius:8px;background:var(--pin-purple);color:#fff;display:flex;align-items:center;justify-content:center;font-size:15px;font-weight:700;flex-shrink:0;">
                        {{ strtoupper(substr($dept ?: 'N', 0, 1)) }}
                    </div>
                    <div>
                        <div style="font-size: 14px; font-weight: 700; color: var(--pin-text);">{{ $dept ?: 'Sans département' }}</div>
                        <div style="font-size: 11px; color: var(--pin-muted); margin-top: 1px;">{{ $employees->count() }} employé{{ $employees->count() > 1 ? 's' : '' }}</div>
                    </div>
                </div>
            </div>
            ...
            <table class="pin-table" style="width: 100%; border-collapse: collapse;">
                ...
                <tbody>
                    ...
                    <tr style="transition:background .12s;">
                        <td style="padding:10px 14px;border-bottom:1px solid var(--pin-border-soft);vertical-align:middle;color:var(--pin-muted);font-size:12px;">{{ $i + 1 }}</td>
                        <td style="padding:10px 14px;border-bottom:1px solid var(--pin-border-soft);vertical-align:middle;">
                            <div style="display:flex;align-items:center;gap:8px;">
                                ...
                                <div>
                                    <div style="font-size:13px;font-weight:500;color:var(--pin-text);">{{ $emp->first_name }} {{ $emp->last_name }}</div>
                                </div>
                            </div>
                        </td>
                        <td style="padding:10px 14px;border-bottom:1px solid var(--pin-border-soft);vertical-align:middle;">
                            <span style="font-size:11px;color:var(--pin-muted);font-family:'SF Mono','Fira Code',monospace;">{{ $emp->matricule ?? '—' }}</span>
                        </td>
                        <td style="padding:10px 14px;border-bottom:1px solid var(--pin-border-soft);vertical-align:middle;">
...
async function regenSingle(empId) {
    ...
    try {
        const res = await fetch('{{ route('pointage.regenerer-pin') }}', {
            method: 'POST',
            headers: {
                'X-CSRF-TOKEN': CSRF,
...
async function doRegenAll(department = null) {
    ...
    try {
        ...
        const res = await fetch('{{ route('pointage.regenerer-tous-pins') }}', {
            method: 'POST',
            headers: {
                'X-CSRF-TOKEN': CSRF,
                'Content-Type': 'application/json',
