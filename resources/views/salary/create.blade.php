...
const EXISTING = {
    ...
    {{-- ⚠️ Nécessite une colonne "overtime_day_override" (boolean) sur la table salaries.
         Si elle n'existe pas encore, exécute une migration avant de mettre ce fichier en prod :
         $table->boolean('overtime_day_override')->default(false); --}}
    overtime_day_hours:       {{ (float) ($existing?->overtime_hours_day ?? $workingData['overtime_day'] ?? 0) }},
    overtime_day_override:    {{ $existing && $existing->overtime_day_override ? 1 : 0 }},
};
...
{{-- ══ CORRECTION : valeur initiale vide — le JS prend le contrôle total ══ --}}
