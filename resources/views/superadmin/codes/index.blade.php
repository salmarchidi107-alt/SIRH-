...
<div class="stats-row">
    <div class="stat-card c-blue">
        ...
        <div class="stat-val blue">{{ number_format($statCounts['assigned']) }}</div>
    </div>
    <div class="stat-card c-green">
        <div class="stat-label">Utilisés (réutilisables)</div>
        <div class="stat-val green">{{ number_format($statCounts['used_once']) }}</div>
    </div>
...
<div class="filters-bar">
    ...
    <span class="f-count" id="fCount">{{ collect($userRowsByTenant)->flatten()->count() }} utilisateur(s)</span>
</div>

{{-- ── Tableau par tenant — 1 ligne par utilisateur ───────────────── --}}
<div id="tenantCards">
...
<div class="tenant-card" data-tenant-id="{{ $tenantId }}">
    <div class="tenant-header" onclick="toggleTenant(this)">
        <div class="tenant-header-left">
            <div class="tenant-avatar" style="background:{{ $tc['bg'] }};color:{{ $tc['color'] }};">{{ $initials }}</div>
            <div>
                <div class="tenant-name">{{ $tName }}</div>
                <div class="tenant-meta">
                    {{ $tTotal }} code(s) · {{ $tAssigned }} attribué(s)
                    @if($tStats)
                        · {{ $tStats['active_employees'] }} employé(s)
                        @if($tStats['missing_count'] > 0)
                            · <strong style="color:var(--amber)">{{ $tStats['missing_count'] }} sans code</strong>
                        @endif
                    @endif
                </div>
            </div>
            ...
            <tbody>
                ...
                <tr class="code-row"
                    ...
                    <td>
                        @if($code->user)
                        <div class="user-row">
                            <div class="user-av" style="background:{{ $tc['bg'] }};color:{{ $tc['color'] }};">
                                {{ strtoupper(substr($code->user->name ?? $code->user->email, 0, 2)) }}
                            </div>
                            <div>
...
<td style="font-size:11px;color:{{ $isUsedOnce ? 'var(--green)' : 'var(--muted)' }};font-weight:{{ $isUsedOnce ? '600' : '400' }};">
    @if($isUsedOnce)
        <span title="{{ $code->used_at?->format('d/m/Y à H:i:s') }}">
            {{ $code->used_at?->format('d/m/Y') }}<br>
            <span style="font-size:10px;opacity:.85;">{{ $code->used_at?->format('H:i:s') }}</span>
        </span>
    @else
        —
    @endif
                    ...
                    <td>
                        <div class="actions-cell">
                            {{-- Remplacer : seulement si le code est encore ASSIGNED (actif) --}}
                            @if($code->status === VC::STATUS_ASSIGNED && $code->user_id)
                            <button class="btn-replace" id="rbtn-{{ $code->id }}"
                                    onclick="replaceForUser({{ $code->user_id }}, {{ $code->id }}, '{{ addslashes($code->user?->name ?? '') }}')">
                                <svg fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2.5">
                                    <path stroke-linecap="round" stroke-linejoin="round"
                                          d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"/>
                                </svg>
                            ...
                            @endif
                            {{-- Révoquer : seulement si le code est encore ASSIGNED (actif, utilisé ou non) --}}
                            @if($code->status === VC::STATUS_ASSIGNED && $code->user_id)
                            <button class="btn-revoke"
                                    onclick="confirmRevoke({{ $code->id }}, '{{ addslashes($code->user?->name ?? 'cet utilisateur') }}')">
                                <svg fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2.5">
                                    <path stroke-linecap="round" stroke-linejoin="round"
...
const REVOKE_URLS = {
@foreach($tenants as $tenant)
    @foreach($userRowsByTenant[$tenant->id] ?? [] as $code)
        @if($code->status === \App\Models\VerificationCode::STATUS_ASSIGNED)
        {{ $code->id }}: "{{ route('superadmin.codes.revoke', $code) }}",
        @endif
    @endforeach
@endforeach
};
