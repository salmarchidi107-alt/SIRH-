    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
        <div class="space-y-4">
            ...
            <div>
                ...
                <span class="status-badge {{ $tenant->plan->badgeClass() }}">
                    {{ $tenant->plan->label() }}
                </span>
            </div>
            ...
            <div>
                ...
                <p class="text-lg font-semibold text-gray-900">{{ $tenant->region }}</p>
            </div>

            <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">Secteur</label>
                <p class="text-gray-900">{{ $tenant->sector ?? 'Non spécifié' }}</p>
            </div>
        </div>
        ...
        <div class="space-y-4">
            <div>
                ...
                @if($tenant->admin)
                    <div class="p-4 bg-gray-50 rounded-lg">
                        <p class="font-semibold text-gray-900">{{ $tenant->admin->name }}</p>
                        <p class="text-gray-600 text-sm">{{ $tenant->admin->email }}</p>
                    </div>
                @else
            ...
            <div>
                ...
                <p class="text-2xl font-bold text-blue-600">{{ $tenant->users_count }}</p>
            </div>

            <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">Créé le</label>
                <p class="text-gray-900">{{ $tenant->created_at->format('d/m/Y à H:i') }}</p>
            </div>
            ...
            <div class="flex items-center space-x-4 pt-2">
                ...
                <div>
                    ...
                    <div class="flex items-center space-x-4 mt-1">
                        <div class="w-12 h-12 rounded-lg border-2" style="background-color: {{ $tenant->brand_color }}"></div>
                        <div class="w-12 h-12 rounded-lg border-2" style="background-color: {{ $tenant->sidebar_color }}"></div>
                    </div>
                </div>
            </div>
        </div>
