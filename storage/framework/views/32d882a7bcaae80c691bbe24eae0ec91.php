...
body, .ov-wrap, .ov-wrap * { font-family: Arial, Helvetica, sans-serif; }

.ov-wrap { padding: 0 0 48px; color: var(--slate-800); }
...
.filters-bar form { display: flex; align-items: center; gap: 10px; flex-wrap: wrap; flex: 1; }
...
.fb-select:focus, .fb-input:focus { outline: none; border-color: var(--teal-500); background: var(--white); }
.fb-select { min-width: 200px; }
.fb-input-year { width: 76px; }
.fb-input-date { width: 130px; }

.btn-filter {
    height: 36px; padding: 0 16px; border: none; border-radius: var(--radius-sm);
...
.filter-sep {
    height: 22px; width: 1px; background: var(--slate-200); margin: 0 4px; flex-shrink: 0;
}
.filter-label {
    font-size: 11px; font-weight: bold; color: var(--slate-400);
...
.filter-active-badge {
    ...
    color: var(--teal-700); font-size: 12px; font-weight: bold;
    white-space: nowrap;
}
.filter-active-badge a {
    color: var(--teal-500); text-decoration: none; font-size: 14px; line-height: 1;
}
.filter-active-badge a:hover { color: var(--red-500); }

/* Navigation periode */
.period-nav { display: flex; align-items: center; gap: 6px; margin-left: auto; position: relative; }

.period-nav a {
    display: inline-flex; align-items: center; justify-content: center;
    width: 32px; height: 32px; border-radius: var(--radius-sm);
    border: 1px solid var(--slate-200); background: var(--slate-50);
    color: var(--slate-600); text-decoration: none; font-size: 14px; font-weight: bold;
    transition: all .15s;
}
.period-nav a:hover { background: var(--teal-600); border-color: var(--teal-600); color: var(--white); }
...
.cal-popup-btn {
    ...
    transition: all .15s; position: relative; line-height: 1;
}
.cal-popup-btn:hover { background: var(--teal-600); border-color: var(--teal-600); color: var(--white); }

.cal-popup-overlay { position: fixed; inset: 0; z-index: 998; display: none; }
.cal-popup-overlay.open { display: block; }

.cal-popup {
    position: absolute; top: calc(100% + 8px); right: 0; width: 316px;
...
.cal-popup-title { font-size: 13px; font-weight: bold; text-transform: capitalize; margin-bottom: 6px; }
.cal-popup-switcher { display: inline-flex; background: rgba(255,255,255,.18); border-radius: 5px; padding: 2px; gap: 2px; }
.cal-popup-switcher button { padding: 3px 10px; border: none; border-radius: 4px; background: transparent; color: rgba(255,255,255,.85); font-family: Arial, Helvetica, sans-serif; font-size: 11px; font-weight: bold; cursor: pointer; transition: all .12s; line-height: 1.6; }
.cal-popup-switcher button.active { background: var(--white); color: var(--teal-700); }
.cal-popup-nav { display: flex; align-items: center; gap: 4px; align-self: flex-start; margin-top: 2px; }
.cal-popup-nav button { width: 27px; height: 27px; border: 1px solid rgba(255,255,255,.3); border-radius: var(--radius-sm); background: rgba(255,255,255,.15); color: var(--white); font-size: 13px; font-weight: bold; cursor: pointer; display: inline-flex; align-items: center; justify-content: center; transition: background .12s; font-family: Arial, Helvetica, sans-serif; line-height: 1; }
.cal-popup-nav button:hover { background: rgba(255,255,255,.3); }
.cal-popup-body { padding: 12px; }

.cal-popup-daynames { display: grid; grid-template-columns: repeat(7, 1fr); gap: 2px; margin-bottom: 4px; }
.cal-popup-dayname { text-align: center; font-size: 10px; font-weight: bold; color: var(--slate-400); text-transform: uppercase; padding: 3px 0; letter-spacing: .04em; }
.cal-popup-grid { display: grid; grid-template-columns: repeat(7, 1fr); gap: 2px; }
.cal-popup-cell { aspect-ratio: 1; display: flex; flex-direction: column; align-items: center; justify-content: center; border-radius: 6px; font-size: 12px; font-weight: bold; color: var(--slate-600); background: transparent; position: relative; transition: background .1s; line-height: 1; }
.cal-popup-cell.cp-weekend { color: var(--slate-300); }
.cal-popup-cell.cp-other-month { color: var(--slate-300); }
.cal-popup-cell.cp-planifie { background: var(--teal-50); color: var(--teal-700); border: 1px solid var(--teal-100); }
.cal-popup-cell.cp-planifie::after { content: ''; position: absolute; bottom: 3px; left: 50%; transform: translateX(-50%); width: 4px; height: 4px; border-radius: 50%; background: var(--teal-500); }
.cal-popup-cell.cp-today { background: var(--teal-600) !important; color: var(--white) !important; border: none !important; }
.cal-popup-cell.cp-today::after { background: rgba(255,255,255,.7) !important; }
.cal-popup-legend { margin-top: 10px; padding-top: 8px; border-top: 1px solid var(--slate-100); display: flex; gap: 14px; font-size: 10px; color: var(--slate-400); flex-wrap: wrap; }
.cal-popup-legend-item { display: flex; align-items: center; gap: 4px; }
.cal-popup-legend-dot { width: 8px; height: 8px; border-radius: 50%; flex-shrink: 0; }

.cal-week-label { font-size: 10px; color: var(--slate-400); text-align: center; margin-bottom: 8px; font-weight: bold; text-transform: uppercase; letter-spacing: .04em; }
.cal-week-grid { display: grid; grid-template-columns: repeat(7, 1fr); gap: 4px; }
.cal-week-cell { border-radius: var(--radius-sm); padding: 8px 3px; text-align: center; background: var(--slate-50); border: 1px solid var(--slate-100); }
.cal-week-cell.cw-planifie { background: var(--teal-50); border-color: var(--teal-100); }
.cal-week-cell.cw-today { background: var(--teal-600); border-color: var(--teal-600); }
.cal-week-cell.cw-weekend { background: var(--slate-50); opacity: .55; }
.cal-week-dayname { font-size: 9px; font-weight: bold; text-transform: uppercase; color: var(--slate-400); letter-spacing: .03em; margin-bottom: 4px; line-height: 1; }
...
.cal-week-num { font-size: 15px; font-weight: bold; color: var(--slate-700); line-height: 1.3; }
...
.cal-week-shift { font-size: 8px; color: var(--teal-600); margin-top: 4px; line-height: 1.4; word-break: break-all; }
...
.emp-avatar {
    ...
    color: var(--white); font-size: 18px; font-weight: bold; flex-shrink: 0; letter-spacing: 1px;
}
.emp-info { flex: 1; }
.emp-name { font-size: 16px; font-weight: bold; color: var(--slate-800); }
.emp-sub  { font-size: 12px; color: var(--slate-500); margin-top: 2px; }
.emp-tags { display: flex; gap: 6px; margin-top: 6px; flex-wrap: wrap; }
...
.emp-kpis { display: flex; gap: 24px; margin-left: auto; }
...
.kpi-card { background: var(--white); border: 1px solid var(--slate-200); border-radius: var(--radius-lg); padding: 16px 18px; box-shadow: var(--shadow-sm); border-top: 3px solid transparent; transition: box-shadow .15s; }
...
.kpi-card.cn { border-top-color: var(--shift-normal-dot); }
.kpi-card.cgarde { border-top-color: var(--shift-garde-dot); }
...
.kpi-card.cn .kpi-value { color: var(--shift-normal-text); }
.kpi-card.cgarde .kpi-value { color: var(--shift-garde-text); }
.kpi-sub { font-size: 12px; color: var(--slate-400); margin-top: 4px; }
.prog-bar { height: 4px; background: var(--slate-100); border-radius: 2px; overflow: hidden; margin-top: 8px; }
...
/* ===================================================================
   BLOC RECAP SHIFT TYPE (dans Recapitulatif du mois)
=================================================================== */
.shift-recap-grid {
...
.shift-recap-card.normal {
    background: var(--shift-normal-bg); border: 1px solid var(--shift-normal-border);
}
.shift-recap-card.garde {
    background: var(--shift-garde-bg); border: 1px solid var(--shift-garde-border);
}
.src-header {
...
.normal .src-dot { background: var(--shift-normal-dot); }
.garde  .src-dot { background: var(--shift-garde-dot); }
.normal .src-header { color: var(--shift-normal-text); }
.garde  .src-header  { color: var(--shift-garde-text); }
.src-val {
    font-size: 20px; font-weight: bold; line-height: 1.2;
}
.normal .src-val { color: var(--shift-normal-text); }
.garde  .src-val  { color: var(--shift-garde-text); }
.src-sub { font-size: 11px; color: var(--slate-400); }

/* ===================================================================
   ONGLETS
=================================================================== */
.tabs { display: flex; gap: 0; border-bottom: 2px solid var(--slate-200); margin-bottom: 20px; }
.tab-btn { padding: 10px 20px; border: none; background: none; font-family: Arial, Helvetica, sans-serif; font-size: 13px; font-weight: bold; color: var(--slate-500); cursor: pointer; border-bottom: 2px solid transparent; margin-bottom: -2px; transition: all .15s; white-space: nowrap; }
...
/* ===================================================================
   CARDS GENERIQUES
=================================================================== */
.card { background: var(--white); border: 1px solid var(--slate-200); border-radius: var(--radius-lg); box-shadow: var(--shadow-sm); overflow: hidden; }
.card-header { padding: 13px 18px; border-bottom: 1px solid var(--slate-200); display: flex; align-items: center; justify-content: space-between; font-size: 14px; font-weight: bold; color: var(--slate-700); background: var(--slate-50); }
.card-body { padding: 18px; }

/* ===================================================================
   LEGENDE GRAPHIQUE
=================================================================== */
.chart-legend { display: flex; flex-wrap: wrap; gap: 16px; margin-bottom: 12px; font-size: 12px; color: var(--slate-500); padding: 0 2px; }
...
.chart-legend-line { width: 18px; height: 3px; border-radius: 2px; flex-shrink: 0; position: relative; }
.chart-legend-line::after { content: ''; position: absolute; top: 50%; left: 50%; transform: translate(-50%,-50%); width: 7px; height: 7px; border-radius: 50%; background: inherit; }
.chart-calcs { margin-top: 12px; font-size: 12px; color: var(--slate-500); line-height: 1.9; padding: 10px 14px; background: var(--slate-50); border-radius: var(--radius-sm); border: 1px solid var(--slate-100); }
...
.cal-day-name { text-align: center; font-size: 11px; font-weight: bold; color: var(--slate-400); text-transform: uppercase; letter-spacing: .04em; padding: 5px 0; }
.cal-grid { display: grid; grid-template-columns: repeat(7, 1fr); gap: 4px; }
.cal-cell { border: 1px solid var(--slate-200); border-radius: var(--radius-sm); padding: 7px 8px; min-height: 80px; background: var(--white); position: relative; transition: border-color .12s; font-size: 12px; }
.cal-cell:hover:not(.cal-empty):not(.cal-weekend) { border-color: var(--teal-500); }
...
/* ── Coloration par type de shift dans le calendrier ── */
.cal-cell.cal-garde {
    border-color: var(--shift-garde-border);
    background: var(--shift-garde-bg);
}
.cal-cell.cal-normal-shift {
    border-color: var(--shift-normal-border);
    background: var(--shift-normal-bg);
}

.cal-num { font-size: 13px; font-weight: bold; color: var(--slate-700); line-height: 1; }
.cal-today .cal-num { display: inline-flex; align-items: center; justify-content: center; width: 22px; height: 22px; border-radius: 50%; background: var(--teal-600); color: var(--white); font-size: 12px; }
.cal-short-day { font-size: 10px; color: var(--slate-400); margin-bottom: 5px; }
.cal-hours { font-size: 13px; font-weight: bold; margin-top: 5px; }
.cal-shift { font-size: 10px; color: var(--slate-400); margin-top: 3px; }
.cal-ecart { font-size: 11px; font-weight: bold; margin-top: 2px; }

/* Badge shift type dans cellule calendrier */
.cal-shift-badge {
    display: inline-flex; align-items: center; gap: 3px;
    font-size: 9px; font-weight: bold; padding: 1px 5px; border-radius: 99px;
    margin-top: 3px; letter-spacing: .02em;
}
.cal-shift-badge.garde  { background: var(--shift-garde-bg); color: var(--shift-garde-text); border: 1px solid var(--shift-garde-border); }
.cal-shift-badge.normal { background: var(--shift-normal-bg); color: var(--shift-normal-text); border: 1px solid var(--shift-normal-border); }

.cal-dot { position: absolute; bottom: 6px; right: 7px; width: 7px; height: 7px; border-radius: 50%; }
...
.cal-legend { display: flex; align-items: center; gap: 14px; font-size: 11px; color: var(--slate-500); flex-wrap: wrap; }
...
.day-table thead tr { background: var(--slate-50); border-bottom: 2px solid var(--slate-200); }
.day-table th { padding: 10px 12px; text-align: left; font-size: 11px; font-weight: bold; text-transform: uppercase; letter-spacing: .04em; color: var(--slate-500); white-space: nowrap; }
.day-table th:not(:first-child) { text-align: right; }
.day-table td { padding: 9px 12px; border-bottom: 1px solid var(--slate-100); vertical-align: middle; white-space: nowrap; }
...
.day-table tbody tr.row-garde { background: var(--shift-garde-bg); }
.day-table tbody tr.row-garde td:first-child { border-left: 3px solid var(--shift-garde-dot); padding-left: 10px; }
.day-table tbody tr.row-normal td:first-child { border-left: 3px solid var(--shift-normal-dot); padding-left: 10px; }
...
/* Shift badge dans tableau */
.shift-pill { display: inline-flex; align-items: center; gap: 3px; padding: 2px 7px; border-radius: 99px; font-size: 10px; font-weight: bold; }
.shift-pill.normale { background: var(--shift-normal-bg); color: var(--shift-normal-text); border: 1px solid var(--shift-normal-border); }
.shift-pill.garde   { background: var(--shift-garde-bg);  color: var(--shift-garde-text);  border: 1px solid var(--shift-garde-border); }
.shift-pill-dot { width: 5px; height: 5px; border-radius: 50%; }
.shift-pill.normale .shift-pill-dot { background: var(--shift-normal-dot); }
.shift-pill.garde   .shift-pill-dot { background: var(--shift-garde-dot); }

/* ===================================================================
   SEMAINES + RECAP + EMPLOYES (inchangé de l'original)
=================================================================== */
.weeks-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(260px, 1fr)); gap: 14px; margin-bottom: 22px; }
.week-card { background: var(--white); border: 1px solid var(--slate-200); border-radius: var(--radius-lg); overflow: hidden; box-shadow: var(--shadow-sm); transition: box-shadow .15s; }
.week-card:hover { box-shadow: var(--shadow); }
.week-head { padding: 12px 16px; display: flex; align-items: center; justify-content: space-between; color: var(--white); }
...
.wk-solde  { font-size: 16px; font-weight: bold; background: rgba(255,255,255,.2); padding: 3px 10px; border-radius: var(--radius-sm); }
.week-stats { display: grid; grid-template-columns: repeat(3, 1fr); border-bottom: 1px solid var(--slate-100); }
.wk-stat { padding: 12px 10px; text-align: center; border-right: 1px solid var(--slate-100); }
...
.week-footer { padding: 10px 14px; display: flex; align-items: center; justify-content: space-between; font-size: 12px; }
...
.recap-table th { padding: 10px 14px; font-size: 11px; font-weight: bold; text-transform: uppercase; letter-spacing: .04em; color: var(--slate-500); text-align: right; }
...
.emp-table th { padding: 10px 14px; font-size: 11px; font-weight: bold; text-transform: uppercase; letter-spacing: .04em; color: var(--slate-500); text-align: center; white-space: nowrap; }
...
.mini-avatar { width: 30px; height: 30px; border-radius: 6px; background: linear-gradient(135deg, var(--teal-600), var(--blue-500)); display: inline-flex; align-items: center; justify-content: center; font-size: 11px; font-weight: bold; color: var(--white); margin-right: 8px; flex-shrink: 0; vertical-align: middle; }
.taux-cell { display: flex; align-items: center; gap: 8px; justify-content: center; }
.taux-track { width: 60px; height: 5px; background: var(--slate-200); border-radius: 3px; overflow: hidden; flex-shrink: 0; }
.taux-fill { height: 100%; border-radius: 3px; }
.link-detail { font-size: 12px; font-weight: bold; padding: 4px 10px; border-radius: var(--radius-sm); background: var(--teal-50); color: var(--teal-700); text-decoration: none; border: 1px solid var(--teal-100); white-space: nowrap; transition: all .12s; }
.link-detail:hover { background: var(--teal-600); color: var(--white); border-color: var(--teal-600); }

.dept-banner { border-radius: var(--radius-lg); padding: 18px 22px; display: flex; align-items: center; gap: 14px; margin-bottom: 20px; background: linear-gradient(135deg, var(--teal-600), var(--blue-600)); color: var(--white); box-shadow: var(--shadow); }
.dept-icon-box { width: 46px; height: 46px; background: rgba(255,255,255,.2); border-radius: var(--radius); display: flex; align-items: center; justify-content: center; font-size: 20px; flex-shrink: 0; }
...
.month-recap-table tr.row-sep td { border-top: 2px solid var(--slate-200); background: var(--slate-50); font-weight: bold; font-size: 11px; text-transform: uppercase; letter-spacing: .05em; color: var(--slate-500); padding-top: 12px; }

.empty-state { background: var(--white); border: 1px solid var(--slate-200); border-radius: var(--radius-lg); padding: 60px 24px; text-align: center; box-shadow: var(--shadow-sm); }
...
.section-title { font-size: 14px; font-weight: bold; color: var(--slate-700); margin: 24px 0 12px; padding-bottom: 8px; border-bottom: 2px solid var(--slate-200); }

@media (max-width: 1100px) { .kpi-grid-4, .kpi-grid-5 { grid-template-columns: repeat(2, 1fr); } .grid-2-1 { grid-template-columns: 1fr; } }
@media (max-width: 768px)  { .emp-kpis { display: none; } .weeks-grid { grid-template-columns: 1fr 1fr; } .cal-cell { min-height: 60px; } .cal-hours, .cal-shift, .cal-ecart { display: none; } .cal-popup { width: 290px; right: -60px; } }
@media (max-width: 480px)  { .weeks-grid { grid-template-columns: 1fr; } }
</style>
<?php $__env->stopPush(); ?>
