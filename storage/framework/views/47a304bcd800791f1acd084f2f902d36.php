<?php $__env->startSection('title', 'Trombinoscope'); ?>
<?php $__env->startSection('page-title', 'Trombinoscope'); ?>

<?php $__env->startSection('content'); ?>
<div class="trombino-page">

<div class="trombino-filters">
    <form method="GET" action="<?php echo e(route('trombinoscope')); ?>" class="trombino-filters-form">
        <div class="trombino-filter-group">
            <label>Recherche par nom</label>
            <input type="text" name="search" value="<?php echo e(request('search', '')); ?>" placeholder="Nom, prénom, poste..." class="trombino-filter-search">
        </div>
        <div class="trombino-filter-group">
            <label>Département</label>
            <select name="department" class="trombino-filter-select">
                <option value="">Tous les départements</option>
                <?php $__currentLoopData = $departments; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $dept): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <option value="<?php echo e($dept); ?>" <?php echo e(request('department') == $dept ? 'selected' : ''); ?>><?php echo e($dept); ?></option>
                 <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
            </select>
        </div>
        <button type="submit" class="trombino-filter-btn">
            Filtrer
        </button>
    </form>
    <?php if(request()->hasAny(['search', 'department'])): ?>
        <a href="<?php echo e(route('trombinoscope')); ?>" class="trombino-filter-reset">
            Réinitialiser
        </a>
    <?php endif; ?>
</div>

<div class="trombino-grid">


<?php $__empty_1 = true; $__currentLoopData = $employees; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $employee): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>

<?php
    $pointage = $pointages[$employee->id] ?? null;
    $statusColor = 'gray';
    $statusText = 'Non pointé';

    if ($pointage) {
        if ($pointage->statut === 'present') {
            $statusColor = 'green';
            $statusText = 'Présent';
        } elseif ($pointage->statut === 'absent') {
            $statusColor = 'red';
            $statusText = 'Absent';
        }
    }
?>

<?php if(auth()->user()->role === 'employee' && auth()->user()->employee_id != $employee->id): ?>
    <div class="trombino-card" style="border-color:<?php echo e($statusColor); ?>" onclick="alert('Accès restreint'); return false;">
<?php else: ?>
    <a href="<?php echo e(route('employees.show', $employee)); ?>" class="trombino-card" style="border-color:<?php echo e($statusColor); ?>">
<?php endif; ?>

    <div class="trombino-photo">
        <?php if($employee->photo): ?>
            <img src="<?php echo e($employee->photo_url); ?>" alt="<?php echo e($employee->full_name); ?>">
        <?php else: ?>
            <?php echo e(strtoupper(substr($employee->first_name,0,1).substr($employee->last_name,0,1))); ?>

        <?php endif; ?>
    </div>

    <div class="trombino-name"><?php echo e($employee->full_name); ?></div>
    <div class="trombino-role"><?php echo e($employee->position); ?></div>
    <div class="trombino-dept"><?php echo e($employee->department); ?></div>

    
    <div class="trombino-status" style="color:<?php echo e($statusColor); ?>">
        ● <?php echo e($statusText); ?>

    </div>

    
    <div class="trombino-contact">

        <div class="trombino-contact-row">
            <svg fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                <path stroke-linecap="round" stroke-linejoin="round"
                      d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.686l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" />
            </svg>
            <?php echo e($employee->phone ?: $employee->numero ?: 'N/A'); ?>

        </div>

        <div class="trombino-contact-row">
            <svg fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                <path stroke-linecap="round" stroke-linejoin="round"
                      d="M16 12a4 4 0 10-8 0 4 4 0 008 0zm0 0v1.5a2.5 2.5 0 005 0V12a9 9 0 10-9 9m4.5-1.206a8.959 8.959 0 01-4.5 1.207" />
            </svg>
            <?php echo e($employee->email ?: 'N/A'); ?>

        </div>

    </div>

<?php if(auth()->user()->role === 'employee' && auth()->user()->employee_id != $employee->id): ?>
    </div>
<?php else: ?>
    </a>
<?php endif; ?>

<?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
<div class="trombino-empty">
    Aucun employé trouvé
</div>
<?php endif; ?>

</div>

</div>
<?php $__env->stopSection(); ?>

<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\HP\Medstaff-second-main\resources\views/trombinoscope/index.blade.php ENDPATH**/ ?>