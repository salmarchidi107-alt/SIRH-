...
const VAR_RETENUES = <?php echo e($variableElements->where('type','retenue')->sum('amount')); ?>;
...
<tr style="background:#f0fff4">
    <td colspan="2" style="padding:9px 14px">
        ...
        <?php $__currentLoopData = $variableElements->where('type','gain'); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $ve): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
    ...
    <div class="card mb-4">
        <div class="card-header" style="background:#fef3c7;border-bottom:2px solid #fcd34d">
            <div style="display:flex;justify-content:space-between;align-items:center;gap:8px">
                ...
                <span id="ir-override-badge"
                      style="display:none;background:#fef3c7;border:1px solid #fcd34d;color:#92400e;
                             padding:2px 10px;border-radius:20px;font-size:0.65rem;font-weight:700;white-space:nowrap;">
                    ✎ modifié
                </span>
            </div>
        </div>
            ...
            <table style="width:100%;border-collapse:collapse;font-size:0.83rem">
                ...
                <tr style="background:#fef3c7">
                    <td style="padding:9px 14px">
                        ...
                        <div style="font-size:0.72rem;color:#92400e;margin-top:2px" id="ir-auto-hint"></div>
                    </td>
                    <td style="padding:9px 14px;text-align:right">
                        <input type="number"
                               id="ir-monthly-input"
                               ...
                               value="0"
                               step="0.01" min="0"
                               style="text-align:right;font-weight:700;color:#991b1b;border-color:#fcd34d;background:#fffbeb;border-width:2px"
                               oninput="onIrManualChange()">
                        <div style="display:flex;gap:6px;margin-top:5px" id="ir-reset-wrap" style="display:none">
                            <button type="button" onclick="resetIrAuto()"
                                    style="flex:1;font-size:0.72rem;padding:4px 8px;border:1px solid #fcd34d;background:white;color:#92400e;border-radius:6px;cursor:pointer;font-weight:600">
                                ↺ Auto
                            </button>
                        </div>
                    </td>
                    ...
                    <tr style="border-bottom:1px solid var(--border-color)">
                        <td style="padding:9px 14px">
                            <div style="font-weight:600;display:flex;align-items:center;gap:6px;flex-wrap:wrap">
                                Absences non payées
                                <span id="absence-override-badge"
                                      style="display:none;background:#fef3c7;border:1px solid #fcd34d;color:#92400e;
                                             padding:1px 8px;border-radius:20px;font-size:0.65rem;font-weight:700;">
                                    ✎ modifié
                                </span>
                        ...
                        <td style="padding:9px 14px;text-align:right">
                            <input type="number"
                                   ...
                                   value="0"
                                   step="0.01" min="0"
                                   style="text-align:right;font-weight:600;color:#991b1b;border-color:#fecaca;border-width:2px;background:#fff8f8"
                                   oninput="onAbsenceManualChange()">
                            <div style="display:flex;gap:6px;margin-top:5px" id="absence-reset-wrap" style="display:none">
                                <button type="button" onclick="resetAbsenceAuto()"
                                        style="flex:1;font-size:0.72rem;padding:4px 8px;border:1px solid #fecaca;background:white;color:#991b1b;border-radius:6px;cursor:pointer;font-weight:600">
                                    ↺ Auto
                                </button>
                            </div>
                        </td>
...
<tr style="background:#fff0f0">
    <td colspan="2" style="padding:9px 14px">
        ...
        <?php $__currentLoopData = $variableElements->where('type','retenue'); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $ve): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
...
/* ══════════════════════════════════════════════════════════════════
   ABSENCE — override
══════════════════════════════════════════════════════════════════ */
function onAbsenceManualChange() {
    var v = parseFloat(document.getElementById('absence-input').value) || 0;
    absenceManual     = v;
    isAbsenceOverride = true;
    updateAbsenceDisplay(v);
    calculate();
}

function resetAbsenceAuto() {
    isAbsenceOverride = false;
    absenceManual     = absenceAutoValue;
    document.getElementById('absence-input').value = absenceAutoValue.toFixed(2);
    updateAbsenceDisplay(absenceAutoValue);
    calculate();
}

function updateAbsenceDisplay(amount) {
    var inp = document.getElementById('absence-input');
    if (inp) {
        inp.style.borderColor = isAbsenceOverride ? '#f59e0b' : '#fecaca';
        inp.style.background  = isAbsenceOverride ? '#fffbeb' : '#fff8f8';
    }
    var badge = document.getElementById('absence-override-badge');
    if (badge) badge.style.display = isAbsenceOverride ? 'inline-flex' : 'none';
    var wrap = document.getElementById('absence-reset-wrap');
    if (wrap) wrap.style.display = isAbsenceOverride ? 'flex' : 'none';
    document.getElementById('h_absence_deduction').value = amount.toFixed(2);
}

/* ══════════════════════════════════════════════════════════════════
   IR — override
══════════════════════════════════════════════════════════════════ */
function onIrManualChange() {
    var v = parseFloat(document.getElementById('ir-monthly-input').value) || 0;
    irManual     = v;
    isIrOverride = true;
    updateIrDisplay(v);
    calculate();
...
function resetIrAuto() {
    isIrOverride = false;
    irManual     = irAutoValue;
    document.getElementById('ir-monthly-input').value = irAutoValue.toFixed(2);
    updateIrDisplay(irAutoValue);
    calculate();
}

function updateIrDisplay(amount) {
    var inp = document.getElementById('ir-monthly-input');
    if (inp) {
        inp.style.borderColor = isIrOverride ? '#f59e0b' : '#fcd34d';
        inp.style.background  = isIrOverride ? '#fef9c3' : '#fffbeb';
    }
    var badge = document.getElementById('ir-override-badge');
    if (badge) badge.style.display = isIrOverride ? 'inline-flex' : 'none';
    var wrap = document.getElementById('ir-reset-wrap');
    if (wrap) wrap.style.display = isIrOverride ? 'flex' : 'none';
    var hint = document.getElementById('ir-auto-hint');
    if (hint) hint.textContent = isIrOverride ? 'Valeur modifiée manuellement — cliquer ↺ Auto pour recalculer' : '';
    document.getElementById('h_ir_deduction').value = amount.toFixed(2);
}
...
function calculate() {
    ...
    // ── ABSENCE : calcule l'auto mais PROTÈGE la valeur manuelle ──
    absenceAutoValue = tauxH * absH;
    var absDeduction;
    if (isAbsenceOverride) {
        absDeduction = absenceManual;
    } else {
        absenceManual = absenceAutoValue;
    ...
    // ── IR : calcule l'auto mais PROTÈGE la valeur manuelle ──
    var irAnnuelBrut, deductFam, irMensuelAuto;
    if(currentSystem==='MAD'){
        ...
        irMensuelAuto  = Math.max(0,irAnnuelBrut-deductFam)/12;
        ...
        deductFam     = 0;
        irMensuelAuto = irAnnuelBrut;
        setHTML('ir-annual',fmt(irAnnuelBrut));
        document.getElementById('ir-family').innerHTML='-0,00 <span class="cur-label">MRU</span>';
    }

    irAutoValue = irMensuelAuto;
    var irMensuel;
    if (isIrOverride) {
        irMensuel = irManual;
    } else {
        irManual  = irMensuelAuto;
        irMensuel = irMensuelAuto;
        var irInp = document.getElementById('ir-monthly-input');
        if (irInp && document.activeElement !== irInp) {
            irInp.value = irMensuelAuto.toFixed(2);
        }
    }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    setTimeout(function(){
        ...
        // Restaurer garde manuelle
        if(isGardeOverride && gardeManual > 0){
            updateGardeDisplay(gardeManual);
            syncGardeToVE(gardeManual);
        }

        // Restaurer absence manuelle
        if(isAbsenceOverride && absenceManual > 0){
            document.getElementById('absence-input').value = absenceManual.toFixed(2);
            updateAbsenceDisplay(absenceManual);
        }
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\HP\Medstaff-second-main\resources\views/salary/create.blade.php ENDPATH**/ ?>